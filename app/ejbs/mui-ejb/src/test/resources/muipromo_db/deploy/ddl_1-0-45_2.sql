...
/* VERIFICO CHE ESISTA IL BALCKUP (SE v_TABLE_NAME รจ null tutto se no sulla specifica tabella )*/
...
IF v_PROCEDI > 0 THEN
  ...
  IF (v_TABLE_NAME = '*' OR v_TABLE_NAME = 'CONFIGURATION') THEN
    ...
    INSERT INTO CONFIGURATION (   ID_CONFIGURATION,    PATH,    JSON,    TYPE,    ID_MENU,    LAST_REVISION_DATE,    LAST_EDITOR  )
    SELECT                        ID_CONFIGURATION,    PATH,    JSON,    TYPE,    ID_MENU,    LAST_REVISION_DATE,    LAST_EDITOR
  ...
  IF (v_TABLE_NAME = '*' OR v_TABLE_NAME = 'USERS') THEN
    ...
    INSERT INTO USERS        (  ID,  NAME,  FILTERS,  ROLE_ID,  HIDDEN_COLS,  CAM_LOGIN,  NAMESPACE_LOGIN,  ADMIN_FILTERS,  DESCRIPTION,  OLD_ROLE_ID ,INGRID_FILTERS)
    SELECT                      ID,  NAME,  FILTERS,  ROLE_ID,  HIDDEN_COLS,  CAM_LOGIN,  NAMESPACE_LOGIN,  ADMIN_FILTERS,  DESCRIPTION,  OLD_ROLE_ID ,INGRID_FILTERS
    FROM USERS_BACKUP
    WHERE  BACKUP_ID  = v_BACKUP_ID_RESTORED ;
