...
public class DateTimeUtils implements Serializable {
    ...
    public LocalDateTime dateAt(@NonNull final LocalDateTime date, @NonNull final String time) throws Exception {
        String[] s = time.split(":");
        if (s.length != 2) {
            throw new IllegalArgumentException("the value " + time + " in not in the expeted format " + TIME_PATTERN);
        }
     ...
     * @return
     */
    public String calculateExtendedDescription(@NonNull final String codicePromozione, @NonNull final Date dataInizio,
                                               @NonNull final Date dataFine, String descrizione) {
        ...
        final long diffInMillies = Math.abs(dataFine.getTime() - dataInizioCalendar.getTime().getTime());
        ...
        return String.format("[%s %s - %sgg] %s", codicePromozione,
                formatDate.substring(0, 1).toUpperCase().concat(formatDate.substring(1)), promoDays, descrizione);
    }
    ...
    public Date toDate(String dateInput, SimpleDateFormat sdf) {
        ...
        try {
            ...
            log.error(String.format("error converting %s to date with format %s", dateInput, sdf.toPattern()), e);
            return null;
        }
    }
    ...
    public boolean compatibleDates(@NonNull final Date date, @NonNull final Date newDate, boolean inizio) {
        if (inizio) {
    ...
    public Integer daysBetween(@NonNull final Date startDate, @NonNull final Date endDate) {
        if (isAfter(startDate, endDate, true)) {
            throw new IllegalArgumentException("startDate must be before endDate");
        }
        LocalDate localStartDate = startDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
        LocalDate localEndDate = endDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
        return (int) Math.abs(ChronoUnit.DAYS.between(localStartDate, localEndDate));
    }
}
